...
Ext.define('Ext.event.publisher.TouchGesture', {
    ...
    constructor: function(config) {
		...
		// ENG-48612 support Surface touch gestures
		this.lastTouchStart = 0;
		this.lastTouchEnd = 0;
		this.mouseEventDismissThreshold= 200; // ms
    ...
    onEvent: function(e) {
		...
		// ENG-48612 support Surface touch gestures
		// essentially both sets of events (tap and mouse) are fired when tap
		// is used in Surface chrome, so we'll dismiss the extra mouse events
		if (type === 'touchstart') {
			this.lastTouchStart = new Date().getTime();
		} else if (type === 'touchend') {
			this.lastTouchEnd = new Date().getTime();
		} else if (type === 'mousedown') {
			if (new Date().getTime() - this.lastTouchStart < this.mouseEventDismissThreshold) {
				return;
			}
		} else if (type === 'mouseup') {
			if (!this.isStarted && new Date().getTime() - this.lastTouchEnd < this.mouseEventDismissThreshold) {
				return;
			}
		}
